...
public class EditRecipeActivity extends AppCompatActivity implements View.OnClickListener, IngredientListener, EasyPermissions.PermissionCallbacks {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recipe_id = getIntent().getIntExtra("id", -1);
    ...
    private void getRecipeDetail() {
        recipeViewModel.getRecipeDetailWithParam(recipe_id, userLocalDatabase.getInt("id", -1)).observe(this, new Observer<RecipeDetailResponse>() {
            @Override
            public void onChanged(RecipeDetailResponse recipeDetailResponse) {
                if (recipeDetailResponse != null) {
                    if (recipeDetailResponse.getRecipeDetail() != null) {
                        ...
                        ingredients = recipeDetailResponse.getListIngredients();
                        steps = recipeDetailResponse.getListSteps();
                        setRecipeDetail();
    ...
    private void setRecipeDetail() {
        category_select = recipe.getCategoryId();
        category_selected.setTextColor(getResources().getColor(R.color.text_color));
        category_selected.setText(recipe.getCategoryDisplay());

        activityAddRecipeBinding.cookTime.setText(recipe.getCookTime());

        if (!recipe.getImage().equals("")){
            setImageURL(activityAddRecipeBinding.recipeImage, recipe.getImage());
        }else{
            activityAddRecipeBinding.recipeImage.setImageResource(R.drawable.noimage);
        }

        activityAddRecipeBinding.edtName.setText(recipe.getName());
        activityAddRecipeBinding.edtOrigin.setText(recipe.getOrigin());
        activityAddRecipeBinding.edtServes.setText(Integer.toString(recipe.getServes()));
        activityAddRecipeBinding.calories.setText(Float.toString(recipe.getCalories()));
        activityAddRecipeBinding.fat.setText(Float.toString(recipe.getFat()));
        activityAddRecipeBinding.protein.setText(Float.toString(recipe.getProtein()));
        activityAddRecipeBinding.carbohydrate.setText(Float.toString(recipe.getCarbo()));
    }
            ...
            case R.id.btn_next_step:
                if (checkAllField()) {
                    ...
                    if (images_picker.size() > 0) {
                        ...
                        recipes.setImageInput(new ArrayList<>());
                    }
                    ...
                    recipes.setUpdateUser(userLocalDatabase.getInt("id", -1));

                    RecipeDataRequest data = new RecipeDataRequest();
                    data.setListInfgredients(ingredients);
                    data.setRecipe(recipes);
                    data.setListSteps(steps);

                    Intent intent2 = new Intent(EditRecipeActivity.this, EditCookingStepsActivity.class);
    ...
    public void onIngredientRemove(int id) {
        ...
        this.ingredientAddAdapter.notifyItemRangeChanged( id , ingredients.size());
    }
    ...
    private String encodeBitmapImage(Bitmap bitmap) {
        ...
        return Base64.encodeToString(bytesofimage, Base64.NO_WRAP);
    }
}